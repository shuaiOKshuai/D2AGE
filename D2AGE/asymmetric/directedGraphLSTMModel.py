...
def directedGraphLSTMModel(options, tparams, x, mask, wemb, buffer_tensor, nodesLen):
    ...
    rval, update=theano.scan(
                             ...
                             outputs_info=[tensor.alloc(numpy_floatX(0.), length, options['dimension']),# @UndefinedVariable 
                                           tensor.alloc(numpy_floatX(0.), length, options['dimension'])],# @UndefinedVariable 
                             )
