...
def prepareDataForTestForSubgraphSingleSequenceWithLengths(query,candidate,subgraphs_map,dimension):
    """
    prepare data for test
    """
    key1=bytes(query)+'-'+bytes(candidate)
    key2=bytes(candidate)+'-'+bytes(query)
    if key1 not in subgraphs_map and key2 not in subgraphs_map:
        return None,None,None
    subgraphs=[] 
    if key1 in subgraphs_map:
        subgraphs.append(subgraphs_map[key1]) 
    if key2 in subgraphs_map:
        subgraphs.append(subgraphs_map[key2]) 
    maxlen=0 
    nsamples=0 
    for value in subgraphs: 
        for sequence in value[1]:
            ...
            if maxlen<len(sequence):
                maxlen=len(sequence)
    sequences=numpy.zeros((nsamples, maxlen)).astype('int64') # shape=nsamples*maxlen
    mask=numpy.zeros((nsamples, maxlen, maxlen)).astype(theano.config.floatX)  # @UndefinedVariable # shape=nsamples*maxlen*maxlen
...
def generateSequenceAndMasksForSingleSequenceWithLength(tuples, tupleFourPairs, subgraphs, dimension):
    """
        generate sequence and masks
    """
    maxlen=0 
    graphNum=0 
    ...
    lens=numpy.zeros((graphNum,)).astype('int64') 
    ...
    for i in range(graphNum): 
        for j in range(maxlen):
            if mask[i][j].sum()==0: 
                mask[i][j][j]=1. 
    buffer_tensor=numpy.zeros([maxlen, maxlen, dimension]).astype(theano.config.floatX)  # @UndefinedVariable 
    for i in range(maxlen):
        for j in range(dimension):
            buffer_tensor[i][i][j]=1.
