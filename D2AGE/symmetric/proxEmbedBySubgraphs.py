...
def init_sharedVariables(options):
    ...
    w = numpy.random.rand(options['dimension'], ).astype(config.floatX)  # @UndefinedVariable 
...
def proxEmbedBySubgraphs(
    ...
    for eidx in range(max_epochs):
        for _, batch in allBatches: 
            ...
            tuples3DMatrix_data, x_data, mask_data, lens_data, subgraph_lens_data, buffer_tensor_data, nodesLens_data=dataProcessTools.generateSequenceAndMasksForSingleSequenceWithLength(trainingDataForBatch, trainingPairsForBatch, subgraphs, dimension)
